...
export default function ChiTietHoaDonNhapHang(props) {
    //
    const {id} = useParams()
    ...
    const getReceipt = async () => {
        setLoading(true);
        const response = await fetch(`https://localhost:7075/api/v1/vendor-receipts/${id}`, {
            headers: {'Content-Type': 'application/json'},
            credentials: 'include',
            method: "GET"
        });
    ...
    return (
        <>
            <div className="container pt-1 m-auto">
                <h1 className="text-center fw-bold">Thông tin hóa đơn nhập hàng</h1>
