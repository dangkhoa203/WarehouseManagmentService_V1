...
export default function NewHoaDonMuaHang(props) {
    //
        ...
        if (checkData(data)) {
            try {
                ...
                const response = await fetch('https://localhost:7075/api/v1/customer-receipts', {
                    ...
                    credentials: 'include',
                    body: JSON.stringify({customerId: data.customerId, dateOrder: data.dateOrder, details: list})
                })
                if (!response.ok) {
                    const text = await response.text();
                    setError(text)
    ...
    return (
        <div className="d-flex justify-content-center flex-column pt-0 p-5 w-100 ">
            <form className="w-100 d-flex justify-content-center flex-column"
                ...
                <p className="h1 text-center mb-3">Tạo hóa đơn mua hàng </p>
                <div className="d-flex flex-column">
                    <div className="row">
                        <div className="col-6">
                            <div className="form-floating mb-3">
                                <select className="form-select" id="floatingSelect"
                                        aria-label="Floating label select example" {...register("customerId")}>
                                    <option value="" selected disabled>Lựa chọn khách hàng</option>
                                    {customerList.map(customer =>
                                        <option value={customer.id}>{customer.name} ({customer.id})</option>
                                    )}
                                </select>
                                <label htmlFor="floatingSelect">Khách hàng</label>
                            </div>
                        </div>
                        <div className="col-6">
                            <div className="form-floating mb-3">
                                <input type={"date"} className="form-control rounded-0 border-2"
            ...
            <div>
                {!addMode ?
                    <div>
                        <button type={"button"} className="btn btn-outline-primary border-3 fw-bold mb-3 w-25"
                                onClick={() => setAddMode(true)}>Thêm
                            sản phẩm vào hóa đơn
                        </button>
                    </div>
                    :
                    <div>
                        <button className="btn btn-outline-secondary border-3 fw-bold w-25"
                        ...
                        <div className="d-flex pt-3 pb-3 gap-5">
                            ...
                            <div className="d-flex justify-content-center w-25">
                                <button className="btn btn-outline-success border-3 fw-bold m-auto w-100"
