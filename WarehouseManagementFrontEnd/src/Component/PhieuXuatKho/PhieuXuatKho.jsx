...
export default function PhieuXuatKho(props){
    ...
    const COLUMNS = [
        ...
        {label: 'Ngày hoàn tiền', renderCell: (item) => new Date(item.orderDate).toLocaleString('En-GB', {
                year: "numeric",
                month: "2-digit",
                day: "2-digit",
                hour12: false
        ...
        { label: 'Hóa đơn mua hàng', renderCell: (item) => <Link className="link link-underline-opacity-0 fw-bolder" to={`/HoaDonNhapHang/${item.receipt.id}`}>{item.receipt.id} </Link>, sort: { sortKey: "idhoadon" } },
    ];
    ...
    return (<>
        <div className="p-5 pt-0">
            <h1 className="pt-4 text-center">Danh sách phiếu xuất kho</h1>
            <Link className="btn btn-outline-primary border-2 fw-bold mb-2" to="tao"><i className="bi bi-plus-circle"> Tạo thêm phiếu xuất kho</i></Link>
