...
export default function ChitietNhomSanPham(props) {
    const {id} = useParams()
    ...
    const getDs = async () => {
        setLoading(true);
        const response = await fetch(`https://localhost:7075/api/v1/Product-Groups/${id}`, {
            headers: {'Content-Type': 'application/json'},
            credentials: 'include',
            method: "GET"
        });
    ...
    return (
        <>
            <div className="container pt-1 m-auto">
                <h1 className="text-center fw-bold">Chi tiết nhóm sản phẩm</h1>
