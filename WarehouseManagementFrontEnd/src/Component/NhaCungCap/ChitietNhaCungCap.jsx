...
export default function ChitietNhaCungCap(props) {
    ...
    const getVendor = async () => {
        setLoading(true);
        const response = await fetch(`https://localhost:7075/api/v1/Vendors/${id}`, {
            headers: {'Content-Type': 'application/json'},
            credentials: 'include',
            method: "GET"
        });
    ...
    return (
        <>
            <div className="container pt-1 m-auto">
                <h1 className="text-center fw-bold">Thông tin nhà cung cấp</h1>
                    ...
                    {!loading ?
                        <>
                            <div
                                ...
                                <div className="col-4">
                                    <h2>Ngày tạo</h2>
                                    <p>{new Date(vendor.createDate).toLocaleString('En-GB', {hour12: false})}</p>
                                </div>
                                <hr/>
                                <div className="col-4">
                                    <h2>Địa chỉ</h2>
